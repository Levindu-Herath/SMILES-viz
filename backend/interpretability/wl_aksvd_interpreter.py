...
class WLAKSVDInterpreter:
    ...
    def __init__(
        ...
        if not hasattr(wl, "vocab"):
            raise AttributeError("WL instance is missing: ['vocab']. Fit or load a vocab first.")

        # class_df / class_counts are only produced by a WL instance that was
        # trained with per-class document-frequency tracking. A WL loaded from
        # a saved artifact bundle (WL.load()) only persists `vocab`, so these
        # default to empty -- Level 1 (prediction explanation / node importance,
        # i.e. the heatmaps) never touches them; only Level 2's class-prevalence
        # columns degrade to 0% when they're absent.
        self.wl = wl
        self.aksvd = aksvd
        ...
        self.class_df: Dict = getattr(wl, "class_df", {})
        self.class_counts: Counter = getattr(wl, "class_counts", Counter())
