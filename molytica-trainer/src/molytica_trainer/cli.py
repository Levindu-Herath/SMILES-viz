...
def main(host: str, port: int):
    """Start the molytica-trainer local server."""
    click.echo(f"Starting molytica-trainer server at http://{host}:{port}")
    uvicorn.run("molytica_trainer.server.app:create_app", host=host, port=port, factory=True)
